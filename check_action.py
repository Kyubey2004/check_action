...
def find(path,need):
    ...
    for root, dirs, files in os.walk(path):
        for file in files:
            ...
            if (file == need or file.startswith(f'{need}.')) and file_date == today and file_size >= 1024:
...
def main():
    # path = input("文件路径")
    # need = input('文件名')
    path='D:\乱七八糟\others\Chaojiying_Python\chaojiying_Python'
    need='1.txt'
    ...
    if result :
        ...
        try:
            ...
            send_email(f"{need}的大小为{target_size / 1024:.2f} KB", "fail", receiver_list=receiver_list)
